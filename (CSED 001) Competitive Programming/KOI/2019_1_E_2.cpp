...
int palindrome(int left, int right, bool pseudo){
    while(left<=right){
        if(s[left]!=s[right]){
            if(pseudo){
                return 2;
                ...
                if(palindrome(left+1, right, true)==1 || palindrome(left, right-1, true)==1) return 1;
                else return 2;
            }
        }
        left++;
...
int main(){
    ...
    for(int i=0; i<T; i++){
        cin>>s;
        cout<<palindrome(0, s.size()-1, false)<<"\n";
    }
