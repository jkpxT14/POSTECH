...
int main(){
    ...
    auto decimal_to_binary=[](int num) -> string {
        if(!num){
            return "0";
        }
        string s;
        while(num>0){
            s.push_back(char(int('0')+(num&1)));
            num>>=1;
        }
    ...
    string in_advance;
    int i(0);
    while(in_advance.size()<=500){
        in_advance+=decimal_to_binary(i);
        ++i;
    }
    in_advance.insert(0, "2");
    
    for(int i(0); i<=4; ++i){
        cout<<in_advance[k+i*n]<<' ';
    }
