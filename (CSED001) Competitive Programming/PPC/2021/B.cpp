...
int main(){
    ...
    ll N, M, K;
    cin>>N>>M>>K;
    vpllll cases{{0, 0}, {D-(N%D), 0}, {0, D-(M%D)}, {D-(N%D), D-(M%D)}};
    pllll ans{0, 0};
    for(pllll p: cases){
        if((((N+p.first)/D)+((M+p.second)/D)+((K-p.first-p.second)/D))>ans.second){
            ans.first=K-p.first-p.second;
            ans.second=(((N+p.first)/D)+((M+p.second)/D)+((K-p.first-p.second)/D));
        } elif((((N+p.first)/D)+((M+p.second)/D)+((K-p.first-p.second)/D))==ans.second && ans.first<K-p.first-p.second){
            ans.first=K-p.first-p.second;
        }
    }
    cout<<ans.first;
