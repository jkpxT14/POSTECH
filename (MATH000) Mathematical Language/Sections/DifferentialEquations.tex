...
$F(t,y,y',\dots,y^{(n)})=0$ & General $n$th-order ordinary differential equation. \\
$y^{(n)}=f(t,y,\dots,y^{(n-1)})$ & $n$th-order ordinary differential equation in normal form. \\
...
\addlinespace[0.26em]
$\displaystyle y=\sum_{n=0}^{\infty}a_n(x-x_0)^n$ & Power-series form about the ordinary point $x_0$. \\
$\displaystyle y=\sum_{n=0}^{\infty}a_n(x-x_0)^{n+r}$ & Frobenius-series form about a regular singular point $x_0$; $r$ denotes an indicial exponent. \\
\addlinespace[0.26em]
...
$\mathbf{x}_e$ & Equilibrium point; $\mathbf f(\mathbf{x}_e)=\mathbf0$. \\
...
\addlinespace[0.26em]
$-(py')'+qy=\lambda wy$ & Sturm--Liouville eigenvalue equation, where $p$, $q$, and $w$ are functions of $x$. \\
$\lambda_n,\ \phi_n$ & Sturm--Liouville eigenvalue and corresponding eigenfunction. \\
...
$G(x,\xi),\ G(\mathbf x,\boldsymbol\xi)$ & Green's function; arguments are scalar or vector as appropriate. \\
\addlinespace[0.26em]
$\operatorname{Ai}(x),\ \operatorname{Bi}(x)$ & Airy functions. \\
$J_\nu(x)$ & Bessel function of the first kind of order $\nu$. \\
$Y_\nu(x),\ N_\nu(x)$ & Bessel function of the second kind of order $\nu$; $Y_\nu$ is preferred. \\
...
$L_n^{(\alpha)}(x),\ L_n(x)$ & Generalized and ordinary Laguerre polynomials; $L_n(x)=L_n^{(0)}(x)$. \\
