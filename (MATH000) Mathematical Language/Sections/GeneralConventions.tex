\section{General Conventions}

These conventions apply throughout. Distinct typographic forms are assigned distinct roles, and clarity takes precedence over brevity. When multiple notations are listed for the same concept, the first is used throughout.

\begin{ruletable}
Scalars & Ordinary mathematical italic letters, for example $a,b,c,\lambda,t$. \\
Vectors & Bold lowercase letters, for example $\mathbf{u},\mathbf{v},\mathbf{x}$. Unless otherwise stated, vectors are column vectors. \\
Matrices & Bold uppercase letters, for example $\mathbf{A},\mathbf{B},\mathbf{M}$. \\
Number systems & Blackboard bold, for example $\N,\Z,\Q,\R,\C$. \\
Sets & Ordinary uppercase letters such as $A,B,X,Y$, unless a field-specific convention requires otherwise. \\
Families and collections & Calligraphic letters when useful, for example $\mathcal{F},\mathcal{T},\mathcal{B}$. \\
Named operators & Upright Roman type, for example $\det$, $\ker$, $\im$, $\rk$, and $\tr$. \\
Large operators & Display size in displayed mathematics and notation tables, with limits above and below when appropriate. \\
Greek variants & Distinct glyphs are treated as distinct symbols; in particular, $\varepsilon$ and $\epsilon$ are not interchangeable. \\
\end{ruletable}

\begin{notationtable}
$(a_1,\dots,a_n)$ & Ordered tuple; parentheses are also the default matrix delimiters. \\
$[a,b]$ & Closed interval. \\
$\{a_1,\dots,a_n\}$ & Set delimiters. \\
$\{x\in A\mid P(x)\}$ & Set-builder notation. \\
$\langle u,v\rangle$ & Pairing or inner product, according to context. \\
$|x|$ & Absolute value, modulus, cardinality, or order, according to context. \\
$\|x\|$ & Norm. \\
\textnormal{s.t.} & Abbreviation for ``such that'' when naturally required. \\
\addlinespace[0.26em]
$a=b$ & Equality. \\
$a\coloneqq b$ & $a$ is defined to be $b$. \\
$a\equiv b$ & Identity, equivalence, or congruence, according to context. \\
$a\cong b$ & Isomorphism or congruence, according to context. \\
$a\simeq b$ & Context-dependent equivalence when standard in the area. \\
$a\approx b$ & Approximation. \\
\addlinespace[0.26em]
$f\colon X\to Y$ & Function or map from $X$ to $Y$. \\
$x\mapsto f(x)$ & Elementwise rule of a map. \\
$g\circ f$ & Composition: first $f$, then $g$. \\
$f|_A,\ f\restriction_A$ & Restriction of $f$ to $A$. \\
$\id_X$ & Identity map on $X$. \\
$\to$ & Map, direction, or field-specific convergence arrow. \\
$\mapsto$ & Elementwise assignment. \\
$\Rightarrow$ & Logical implication. \\
$\Leftrightarrow$ & Logical equivalence. \\
\end{notationtable}
