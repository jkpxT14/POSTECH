...
\date{20260821, Seoul High School Departmental Mentoring}
...
\end{mainitemize}
\vspace{0.25em}
\begin{block}{My GitHub}
{\scriptsize
\url{https://github.com/jkpxT14/POSTECH}\par\medskip
\url{https://github.com/jkpxT14/POSTECH/tree/main/(POKA001)\%20Science\%20Quiz}\par\medskip
\url{https://github.com/jkpxT14/POSTECH/tree/main/(MATH100)\%20Introduction\%20to\%20Mathematics\%2C\%20Department\%20of\%20Mathematics/20260821\%2C\%20Seoul\%20High\%20School\%20Departmental\%20Mentoring}
}
\end{block}
\end{frame}
...
\begin{frame}{POSTECH}
\begin{mainitemize}
  \item SNU, POSTECH, KAIST, KoreaU, YonseiU
  \item 소수정예 \& 연구중심
  \item 전액 장학금
  \item 1000만원 바우처 지급
...
\begin{block}{Recent Achievements of AI in Mathematics}
{\scriptsize
...
\url{https://openai.com/index/ten-advances-in-mathematics/}\par\medskip
\url{https://www.anthropic.com/research/riemann-zeta}
}
\end{block}
\begin{mainitemize}
  \item Olympiad-style 문제는 점령된지 오래이다.
  \item 일부 Research-level 문제까지 해결하고 있다.
\end{mainitemize}
\end{frame}

\begin{frame}{Riemann Hypothesis}
\begin{mainitemize}
  \item 여기에 너가 리만 가설의 정의(명제, statement) 적어줘. 한국어로 적어줘도 좋은데 수학적으로 맞게 기술해줘.
  \item 위에서 리만 가설 수학적으로 잘 적어놓았다면 가장 대표적인 리만가설의 설명을 몇개 적어줘.
  \item 예컨대 밀레니엄 난제인가 그거 해당한다던지
  \item 리만 가설을 풀면 나타날 영향이라던지
  \item 이 문제의 별명이나 악명이 자자하다면 그 이야기라던지
  \item 리만 가설 관련해서 살짝 적어줘. 그리고 최대한 교재의 다른 부분과 말투, 형식 맞추고 맞춤법 맞춰줘.
\end{mainitemize}
\end{frame}

\begin{frame}{Claude's Achievement to Riemann Hypothesis}
\begin{mainitemize}
  \item 여기에서도 너가 67.2\%를 포함해서 실제로 클로드가 해낸 업적이 뭔지 적어줘.
  \item 내가 사이트 줄테니까 제대로 읽어보고 클로드가 한게 뭔지 간단하게 적어줘.
\end{mainitemize}
\end{frame}

\begin{frame}{Mathematics in the age of AI}
\begin{block}{Mathematics in the age of AI, Terence Tao}
{\scriptsize
\url{https://www.youtube.com/watch?v=M0--ZH1lOzg}\par\medskip
\url{https://teorth.github.io/tao-web/slides/age-of-ai-icm-2026.pdf}\par\medskip
\url{https://arxiv.org/html/2608.16753v1}
}
\end{block}
\begin{mainitemize}
  \item 앞으로 수학계는 어떤 방향으로 나아가야 하는가?
\end{mainitemize}
\end{frame}
...
\begin{frame}{If you want to try University Math}
\begin{mainitemize}
  \item 미적분학, 해석학
    \begin{subitemize}
      \item Calculus With Applications, Multivariable Calculus with Applications (Peter D. Lax)
      \item Calculus: Early Transcendentals (James Stewart)
      \item Principles of Mathematical Analysis (Walter Rudin)
    \end{subitemize}
  \item 선형대수학
    \begin{subitemize}
      \item Linear Algebra and Its Applications (Gilbert Strang)
      \item Elementary Linear Algebra (Howard Anton)
    \end{subitemize}
\end{mainitemize}
\end{frame}
...
\begin{mainitemize}
  ...
  \item 2028학년도 대학수학능력시험: 큐레이션. 기본으로 돌아가자. 통합적으로 생각하자.
\end{mainitemize}
\end{frame}
