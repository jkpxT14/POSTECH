...
리만 제타함수 $\zeta(s)$의 모든 비자명한 영점 $s$에 대하여
\[
\operatorname{Re}(s)=\frac{1}{2}.
\]
\end{block}
\begin{mainitemize}
  \item 비자명한 영점은 $0<\operatorname{Re}(s)<1$인 임계띠에 놓이며, 리만 가설은 이들이 모두 그 한가운데인 임계선 $\operatorname{Re}(s)=\frac{1}{2}$ 위에 있다고 주장한다.
  \item 1859년 베른하르트 리만이 제기했으며, 7개의 밀레니엄 문제 중 하나이다.
  \item 160년 넘게 미해결인, 수학에서 가장 유명한 난제 중 하나이다.
  \item 소수의 분포와 깊게 연결되어 있다.
\end{mainitemize}
\end{frame}

\begin{frame}{Claude's Progress on the Riemann Hypothesis}
\begin{block}{Lower Bound}
\[
41.6\% \longrightarrow 67.2\%
\]
\end{block}
\begin{mainitemize}
  \item 임계선 $\operatorname{Re}(s)=\frac{1}{2}$ 위에 놓이는 비자명한 영점의 비율의 하한을 41.6\%에서 67.2\%로 개선했다.
\end{mainitemize}
\end{frame}
...
\begin{mainitemize}
  \item 미적분학, 해석학
    \begin{subitemize}
      \item Calculus With Applications, Multivariable Calculus with Applications (Peter Lax)
