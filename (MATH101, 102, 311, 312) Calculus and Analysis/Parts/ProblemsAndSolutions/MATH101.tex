...
Determine whether each of the following infinite series converges. You do not need to justify your answers.
\begin{multicols}{2}
...
\begin{problem}[2021 Spring \KAIST{} Calculus I]
Let $f$ be a continuously differentiable one-to-one function defined on $[a,b]$. Assume that $f'(x)\ne0$ on $[a,b]$. Compute the following sum of two integrals:
\[
...
\item $x\times(\ln x)^{2022}=o(x^{1.001})$.
...
\begin{problem}[2021 Fall \KAIST{} Calculus I]
\begin{problemsubparts}
\item Derive $\displaystyle\frac{\dd}{\dd x}(\tanh^{-1}x)$ for $|x|<1$.
\item Find $\displaystyle\int x\tanh^{-1}x\,\dd x$.
\end{problemsubparts}
\end{problem}
