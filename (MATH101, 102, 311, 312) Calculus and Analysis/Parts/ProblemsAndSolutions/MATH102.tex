...
\item Assume $f$ is infinitely differentiable at $0$, that is, $f^{(n)}(0)$ exists for all $n=1,2,3,\ldots$. Then there exists $\varepsilon>0$ such that
\[
f(x)=\sum_{n=0}^{\infty}\frac{f^{(n)}(0)}{n!}x^n
\]
for all $-\varepsilon<x<\varepsilon$.
\item Let $A(x)=\displaystyle\sum_n a_nx^n$ and $B(x)=\displaystyle\sum_n b_nx^n$ be power series with radii of convergence $R_1$ and $R_2$, respectively. Then $A(x)\times B(x)$ is again a power series with radius of convergence $\min\{R_1,R_2\}$.
\end{problemchoices}
\end{problem}
