    ...
    \begin{expectedproblem}[Moser's Circle Problem]
        Place $n$ points on a circle and join every pair of points by a chord.
    ...
    \begin{solution}
        Give the rotating line an orientation and call its two open sides the
        left side and the right side. When the pivot changes from $T$ to $U$,
        keep the orientation of the line unchanged. Immediately after the
        change, the old pivot $T$ lies on the same side on which $U$ lay
        immediately before the change. Consequently, except at the instants
        when the line contains two points of $S$, the number of points on each
        side of the oriented line is invariant throughout the process.

        First suppose that
        \[
            |S|=2m+1.
        \]
        Through every point $T\in S$ there is an oriented line having exactly
        $m$ points of $S$ on each side. Indeed, rotate any line through $T$
        through an angle $\pi$. The number of points on its left side changes
        by one whenever the line passes through a point of $S$, and its initial
        and final values add to $2m$; hence the value $m$ occurs.

        Choose such a line through any point $P$ as the initial state. The
        side counts remain $(m,m)$. For any fixed direction, there is a unique
        translate through a point of $S$ that has $m$ points on each side: it
        is the median line in that direction. Hence, whenever the windmill
        reaches the direction of a balanced line through a prescribed point
        $T$, it must pass through $T$, so $T$ is used as a pivot. Thus every
        point is visited during each rotation of the line through an angle
        $\pi$.
        ...
        \[
            |S|=2m.
        \]
        Through every point $T\in S$ there is an oriented line with $m-1$
        points on its left side and $m$ points on its right side. The same
        half-turn argument proves this. Choose one such line as the initial
        state. Its ordered side counts remain $(m-1,m)$. For each oriented
        direction, the line through a point of $S$ with these side counts is
        unique. Therefore, during a full rotation through $2\pi$, the windmill
        passes through every point of $S$.

        The set of directions determined by pairs of points of $S$ is finite,
        and successive pivot changes advance the line by a positive angle.
        Hence the total angle of rotation tends to infinity. Repeating the
        preceding argument over successive half-turns or full turns shows that
    ...
    \begin{expectedproblem}[Art Gallery Problem]
        ...
        polygon. Prove that the entire museum can always be guarded by
        \[
