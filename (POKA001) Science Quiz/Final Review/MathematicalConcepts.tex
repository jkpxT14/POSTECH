...
주값 $\arctan\in(-\pi/2,\pi/2)$에서 $1+uv>0$이면
\[
\arctan u-\arctan v
...
=\arctan\!\left(\frac ax\right)-\arctan\!\left(\frac a{x+1}\right),
\]
이므로 연속한 $x$에 대해 합하면 망원합이 된다.
\end{formula}

\begin{formula}[Logarithmic Moments]
$a>-1$, $m\in\Nzero$이면
\[
\boxed{\displaystyle\int_0^1x^a(\log x)^m\,\dd x
=\frac{(-1)^m m!}{(a+1)^{m+1}}}.
\]
따라서 기하급수와 결합하면
\[
...
Heine--Borel 정리:
\[
\boxed{K\subset\R^n\text{ 콤팩트}\iff K\text{가 닫히고 유계}.}
\]
...
\]
를 반복한다. $\alpha\in\Q$이면 모든 궤도는 주기적이고, $\alpha\notin\Q$이면 각 궤도는 원 위에 조밀하다. ``유리수 $\leftrightarrow$ 주기적, 무리수 $\leftrightarrow$ 조밀''를 한 쌍으로 기억한다.
\end{fact}
...
연속함수열 $f_n$이 $f$에 \textbf{균등수렴}하면 $f$도 연속이다.
\[
\boxed{f_n\rightrightarrows f,\quad f_n\text{ continuous}\Longrightarrow f\text{ continuous}.}
\]
점별수렴만으로는 성립하지 않는다. 미분과 극한의 교환에는 더 강한 가정이 필요하다.
\end{theorem}

\begin{theorem}[Local Uniform Convergence: Useful Consequence]
함수열에서 미분과 극한의 순서를 바꾸려면 균등수렴에 가까운 조건을 확인해야 한다. 기출처럼 ``국소 균등수렴 + 도함수 제어''가 주어지면 점별수렴만 보고 판단하지 않는다.
\end{theorem}
...
\begin{formula}[Roots-of-Unity Product]
$\omega_n=e^{2\pi i/n}$이면
\[
x^n-1=\prod_{k=0}^{n-1}(x-\omega_n^k),
\qquad
\boxed{\prod_{k=1}^{n-1}(1-\omega_n^k)=n}.
\]
두 번째 식은 $(x^n-1)/(x-1)$에 $x=1$을 대입하면 바로 얻는다.
\end{formula}
...
\begin{formula}[Nilpotent Powers]
$J_s(0)$가 크기 $s$인 nilpotent Jordan block이면
\[
\boxed{\rk(J_s(0)^k)=\max(s-k,0)}.
\]
일반 nilpotent matrix의 Jordan block 크기가 $s_1,\ldots,s_r$이면
\[
\rk(\mathbf N^k)=\sum_j\max(s_j-k,0),
\qquad
\dim\ker(\mathbf N^k)=\sum_j\min(k,s_j).
\]
특히 $\rk(\mathbf N)=n-1$이면 Jordan block이 하나이고 $\rk(\mathbf N^k)=\max(n-k,0)$이다.
\end{formula}
...
\begin{formula}[Bessel Equation and Bessel Functions]
원통ㆍ원형 대칭의 변수분리에서 자주 나타나는 Bessel 방정식은
\[
\boxed{x^2y''+xy'+(x^2-\nu^2)y=0}.
\]
대표해는 제1종ㆍ제2종 Bessel 함수 $J_\nu(x),Y_\nu(x)$이다. ``원통 대칭 $\leftrightarrow$ Bessel''을 함께 기억한다.
\end{formula}
...
\]
``적어도 하나''를 직접 세기 복잡하면 여사건을 먼저 본다.
\end{formula}
...
$D_n$을 아무 원소도 제자리에 있지 않는 순열의 수라 하면
\[
...
$\{1,\ldots,n\}$에서 서로 이웃하지 않는 $k$개를 고르는 방법 수는
\[
\boxed{\binom{n-k+1}{k}}.
\]
서로 다른 $n$개를 일렬로 놓을 때 지정된 $r$개가 한 덩어리로 붙어 있어야 하고 내부 순서도 자유라면
\[
\boxed{r!(n-r+1)!}.
\]
간격을 하나씩 제거하거나, 붙어야 하는 대상을 하나의 block으로 본다.
\end{formula}
...
그래프 $G$의 여그래프를 $\overline G$라 하면
\[
\boxed{\omega(G)=\alpha(\overline G)},
\qquad
\boxed{\alpha(G)=\omega(\overline G)},
\]
여기서 $\omega$는 최대 clique 크기, $\alpha$는 최대 independent set 크기이다. clique와 independent set은 여그래프에서 서로 뒤바뀐다.
\end{fact}
...
순서가 중요한가, 중복이 허용되는가, 여사건이 쉬운가, 포함배제가 필요한가, 격자경로로 바뀌는가, 대각선 제약이 Catalan/reflection으로 가는가를 순서대로 본다.
...
\]
\boxed{\text{독립성은 필요 없다.}}
개수의 기댓값을 묻는 문제에서는 지시확률변수 (indicator variable)를 먼저 고려한다.
\end{theorem}

\begin{formula}[Cycles in a Uniform Random Permutation]
$S_n$에서 균등하게 순열을 하나 고르고, $X_r$를 길이 $r$인 순환의 개수라 하면
\[
\boxed{\Exp[X_r]=\frac1r}.
\]
따라서 전체 순환 수 $C_n$에 대해
\[
\boxed{\Exp[C_n]=H_n=1+\frac12+\cdots+\frac1n}.
\]
\end{formula}
...
\boxed{\gcd(a,n)\mid b}.
\]
$d=\gcd(a,n)$가 $b$를 나누면 법 $n$에서 정확히 $d$개의 해의 residue classes가 생긴다.
\end{theorem}
...
\gcd(a,n)=1.
\]
확장 Euclidean algorithm으로 역원을 실제로 구할 수 있다.
\end{formula}
...
x\equiv b\pmod n
\]
은 modulo $mn$에서 유일한 해의 합동류를 가진다. 서로소인 여러 moduli에도 동일하게 확장된다.
\end{theorem}
...
\begin{theorem}[소수정리와 Bertrand 공준]
\[
...
\begin{fact}[Riemann Zeta: Analytic-Continuation Anchor]
Riemann zeta 함수의 해석적 연장을 쓰면
\[
\boxed{\zeta(-1)=-\frac1{12}},
\qquad
\zeta(-2m)=0\quad(m\in\N).
\]
중요: $1+2+3+\cdots$가 보통 의미로 $-1/12$에 수렴한다는 뜻은 아니다.
\end{fact}

\begin{fact}[정수론 주요 암기 단서]
\begin{itemize}
...
\]
위상동형사상 (위상동형)은 연속인 전단사이고 역함수도 연속인 사상이다. 정의보다 아래의 불변량과 유명 정리를 우선 기억한다.
\end{formula}
...
M\"obius strip은 비가향이며 경계가 하나 있다. Klein 병은 닫힌 비가향곡면이고 $\R^3$에 자기교차 없이 매장할 수 없다.
\end{fact}
...
전략게임에서 다른 사람의 전략을 고정했을 때 어느 한 사람도 혼자 전략을 바꾸어 자신의 보상을 높일 수 없는 전략조합을 \textbf{Nash 균형}이라 한다.
\end{definition}

\begin{algorithm}[2×2 영합게임]
행 플레이어의 보상행렬이
\[
...
이고 내부 혼합균형이 존재하면
\[
\boxed{p=\frac{d-c}{\Delta}},\qquad
\boxed{q=\frac{d-b}{\Delta}},\qquad
\boxed{v=\frac{ad-bc}{\Delta}}.
\]
$p$는 첫째 행, $q$는 첫째 열을 고를 확률이고 $v$는 행 플레이어의 게임값이다. 공식보다 먼저 \textbf{순수 안장점 (saddle point)가 있는지} 확인하고, 혼합전략에서는 상대가 두 순수전략 사이에서 무차별하도록 확률을 정한다.
\end{algorithm}



\par\smallskip\noindent\textbf{Vieta jumping.} 양의 정수해 문제에서 한 변수를 근으로 보는 이차방정식을 만든 뒤 Viète 공식으로 다른 정수근을 구성한다. 더 작은 양의 해를 얻으면 최소해 가정과 모순이다. IMO형 정수론의 대표적인 무한강하 전략이다.
