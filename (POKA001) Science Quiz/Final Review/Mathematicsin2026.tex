...
\begin{fact}[상태 표현]
최신 수학에서는 결과의 상태를 반드시 구분한다.
\begin{itemize}
\item \textbf{published}: 학술지 또는 학술 출판물에 정식 게재된 상태.
\item \textbf{accepted}: 게재 승인이 공식적으로 알려졌지만 최종 호(issue)는 아직 나오지 않았을 수 있는 상태.
\item \textbf{preprint}: 정식 게재 전에 공개된 원고.
\item \textbf{announced}: 저자나 책임 있는 기관이 결과를 공개 발표한 상태; publication status는 별도로 확인한다.
\item \textbf{formally verified}: Lean 등의 proof assistant가 형식화된 증명을 검사한 상태; ordinary peer review와는 다른 주장이다.
\item \textbf{scheduled}: 공식 발표된 미래 행사 또는 임명.
\end{itemize}
수치적 증거, 발표, preprint, formal verification, 정식 publication은 서로 바꾸어 말하지 않는다.
\end{fact}
...
\bottomrule\endlastfoot
Navier--Stokes & OpenAI, Sep. 8 announcement & forced 3D finite-time singularity manuscript + Lean formalization 공개; Clay의 공식 인정과 ordinary publication은 별도\\
...
2026년 9월 8일 OpenAI가 finite-time singularity를 보이는 solution manuscript와 Lean formalization을 공개했다. Final에서는
\[
\boxed{\text{해결 발표 + 원고 공개 + Lean 형식화 공개}}
\]
까지를 확인된 상태로 기억한다. formal verification, ordinary publication, Clay의 공식 인정, 그리고 최종 credit determination은 서로 별개의 주장이다.
\end{fact}
...
Sang-il Oum과 Jim Geelen이 독립 expository write-ups를 게시했다. 공개 Lean formalization도 존재하지만,
이 사실들은 ordinary journal publication과 구분한다.
\end{fact}
...
\begin{fact}[Palomar]
2026년 8월 18일 Lean FRO와 ICARM이 incubate한 Lean-verified mathematics registry \textbf{Palomar}가 submissions를 받기 시작했다. Jeremy Avigad, Terence Tao, Ravi Vakil, Akshay Venkatesh 등이 advisory/review 생태계에 참여한다.
...
\]
의 Lean derivation을 다루지만 세 개의 명시적 project input axiom과 numerical certificate에 의존하는 conditional formalization이다. certificate가 그 axioms 자체를 Lean에서 증명하는 것은 아니다. Twin Prime Conjecture가 해결된 것도 아니다.
\end{fact}
