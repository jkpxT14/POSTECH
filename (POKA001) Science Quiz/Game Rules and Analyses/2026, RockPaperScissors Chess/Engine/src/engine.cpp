...
InitialChoiceResult Engine::choose_initial(const SearchLimits& limits) {
    ...
    if (!has_explicit_budget(limits)) {
        ...
        for (bool first : {true, false}) for (int bucket = 0; bucket < 3; ++bucket)
        ...
        for (std::size_t rank = 0; rank < pct.size(); ++rank) {
            ...
            int bucket = choice.lines[rank].bucket;
            choice.lines[rank] = probe_initial(first, bucket, search_, scaled_limits(limits, pct[rank], 100));
        }
    }
    std::stable_sort(choice.lines.begin(), choice.lines.end(), better);
    if (!choice.lines.empty()) {
        const auto& x = choice.lines.front();
        choice.choose_first = x.choose_first; choice.best_bucket = x.bucket;
        choice.chooser_value = x.chooser_value; choice.white_value = x.white_value;
    }
    return choice;
}
