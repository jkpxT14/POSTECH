...
Value terminal_match_value(const Position& position){const int d=position.score(Color::White)-position.score(Color::Black); if(d) return ScoreUnit*d; const int q=position.quiz(Color::White)-position.quiz(Color::Black); return q>0?ScoreUnit/4:q<0?-ScoreUnit/4:0;}
int item_reserve_value(const Position& position){const int r=position.remaining_board_plies(); if(r<0) return BaseItemReserveValue; return std::max(0,std::min(BaseItemReserveValue,2+r));}
}
Value evaluate_white(const Position& position){if(position.remaining_board_plies()==0) return terminal_match_value(position); Value value=ScoreUnit*(position.score(Color::White)-position.score(Color::Black)); value+=12*(position.alive_count(Color::White)-position.alive_count(Color::Black)); int wi=0,bi=0; for(int b=0;b<3;++b){wi+=position.item_count(Color::White,b);bi+=position.item_count(Color::Black,b);} value+=item_reserve_value(position)*(wi-bi); for(int i=0;i<PieceCount;++i){auto id=static_cast<PieceId>(i); if(!position.piece(id).alive()) continue; int mobility=2*first_step_mobility(position,id)+second_step_continuations(position,id)/2; value+=piece_color(id)==Color::White?mobility:-mobility;} return value;}
