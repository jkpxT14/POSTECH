...
void Position::reset_pieces() {
    ...
    const Orientation white_scissors =
        orientations.canonical(Gesture::Scissors, WristDirection::South);
    const Orientation white_rock =
        orientations.canonical(Gesture::Rock, WristDirection::South);
    const Orientation white_paper =
        orientations.canonical(Gesture::Paper, WristDirection::South);
    const Orientation black_scissors =
        orientations.canonical(Gesture::Scissors, WristDirection::North);
    const Orientation black_rock =
        orientations.canonical(Gesture::Rock, WristDirection::North);
    const Orientation black_paper =
        orientations.canonical(Gesture::Paper, WristDirection::North);
...
MoveOutcome Position::do_move(const Move& move, UndoState& undo) {
    ...
    if (adjacent_enemies(moving_piece.square, mover, move.piece) == 1) {
        ...
        const auto& orientations = OrientationTable::instance();
        const int result = combat(orientations.top_gesture(moving_piece.orientation),
                                  orientations.top_gesture(enemy.orientation));

        if (result != 0) {
            outcome.has_capture = true;
            outcome.captured = result > 0 ? enemy_id : move.piece;
...
Key Position::key() const {
    ...
    for (int i = 0; i < PieceCount; ++i) {
        ...
        const std::uint64_t feature =
            (static_cast<std::uint64_t>(i) * 65u + square) * 24u + piece.orientation;
...
Key Position::search_key() const {
    ...
    for (int i = 0; i < PieceCount; ++i) {
        ...
        const std::uint64_t feature =
            (static_cast<std::uint64_t>(i) * 65u + square) * 32u + state;
...
std::string Position::debug_string() const {
    ...
    for (int i = 0; i < PieceCount; ++i) {
        ...
        if (!piece.alive()) {
            ...
            out << char('a' + file_of(piece.square)) << (rank_of(piece.square) + 1) << " top="
