...
int Searcher::alphabeta(Position&p,int d,int a,int b,int ply,bool pv,int ext_used){sel_=std::max(sel_,ply);if(p.remaining_plies==0||ply>=64)return evaluate(p);if(d<=0)return qsearch(p,a,b,ply);if(timed_out())return evaluate(p);int st=evaluate(p);if(!pv&&d<=4){if(st-(60+30*d)>=b)return st;if(d==1&&st+70<=a)return qsearch(p,a,b,ply);}auto k=p.key();const TTEntry*e=tt_.probe(k);int oa=a,ob=b;if(e&&e->depth>=d&&!pv){if(e->bound==0)return e->value;if(e->bound==1&&e->value>=b)return e->value;if(e->bound==-1&&e->value<=a)return e->value;}
 auto ms=generate_moves(p,false);if(ms.empty())return evaluate(p);Move ttmove{};bool has_tt=e&&e->has_best; if(has_tt)ttmove=e->best;std::sort(ms.begin(),ms.end(),[&](auto&A,auto&B){auto score=[&](auto&x){int s=x.tactical>0?500000+x.tactical*10000:x.tactical<0?-250000:0;if(x.reset)s+=180000;if(has_tt&&same_move(x.move,ttmove))s+=1000000;if(x.move.item!=Item::None)s+=500;return s;};return score(A)>score(B);});Move best{};bool hb=false;int j=0;for(auto&x:ms){Position n=p;int alive0=p.alive_total();if(!n.apply(x.move))continue;int alive1=n.alive_total();bool transition=x.tactical!=0&&alive0>2&&alive1==2;int extra=(x.tactical!=0&&ext_used<1)?1:0;if(x.reset&&ext_used+extra<2)extra++;if(transition&&ext_used+extra<4)extra=std::max(extra,4-ext_used);int ne=std::min(4,ext_used+extra),cd=d-1+extra,sc; if(j==0)sc=-alphabeta(n,cd,-b,-a,ply+1,pv,ne);else{bool critical=x.tactical!=0||x.reset||transition;bool reduce=!pv&&!critical&&d>=4&&j>=4;int r=(reduce&&d>=7&&j>=10&&x.move.item==Item::None)?2:1;if(reduce){sc=-alphabeta(n,std::max(0,cd-r),-a-1,-a,ply+1,false,ne);if(sc>a)sc=-alphabeta(n,cd,-a-1,-a,ply+1,false,ne);}else sc=-alphabeta(n,cd,-a-1,-a,ply+1,false,ne);if(sc>a&&sc<b)sc=-alphabeta(n,cd,-b,-a,ply+1,pv,ne);}j++;if(stop_)return a;if(sc>a){a=sc;best=x.move;hb=true;}if(a>=b)break;}
 TTEntry ne;ne.depth=d;ne.value=a;ne.bound=a<=oa?-1:a>=ob?1:0;ne.best=best;ne.has_best=hb;tt_.store(k,ne);return a;}
Searcher::RootPass Searcher::root_pass(const Position&p,const std::vector<ScoredMove>&roots,const Move*preferred,int d,int a,int b,const std::vector<Move>&skip,bool multipv){std::vector<ScoredMove>ord;for(auto&x:roots)if(!skipped(x.move,skip))ord.push_back(x);std::sort(ord.begin(),ord.end(),[&](auto&A,auto&B){auto s=[&](auto&x){int v=x.tactical>0?500000+x.tactical*10000:x.tactical<0?-250000:0;if(x.reset)v+=180000;if(preferred&&same_move(x.move,*preferred))v+=1200000;return v;};return s(A)>s(B);});RootPass r;if(ord.empty())return r;int alpha=a,j=0;for(auto&x:ord){Position n=p;int alive0=p.alive_total();n.apply(x.move);int alive1=n.alive_total();bool transition=x.tactical!=0&&alive0>2&&alive1==2;int extra=x.tactical!=0?1:0;if(x.reset)extra=std::max(extra,2);if(transition)extra=std::max(extra,4);int ext=std::min(4,extra),cd=d-1+extra,v;if(j==0)v=-alphabeta(n,cd,-b,-alpha,1,true,ext);else{bool reduce=!multipv&&d>=3&&x.tactical==0&&j>=64;if(reduce){v=-alphabeta(n,std::max(0,cd-1),-alpha-1,-alpha,1,false,ext);if(v>alpha)v=-alphabeta(n,cd,-alpha-1,-alpha,1,false,ext);}else v=-alphabeta(n,cd,-alpha-1,-alpha,1,false,ext);if(v>alpha&&v<b)v=-alphabeta(n,cd,-b,-alpha,1,true,ext);}j++;if(stop_)break;r.scores.push_back({x.move,v});if(!r.has_best||v>r.value){r.has_best=true;r.best=x.move;r.value=v;}if(v>alpha)alpha=v;if(alpha>=b)break;}return r;}
