...
int root_action_family(const Move& move) {
    ...
    if (is_rotation(move.item)) return 2;
...
struct Search::Context {
    ...
    explicit Context(Search& search)
        : history(search.history_), capture_history(search.capture_history_),
          countermoves(search.countermoves_), continuation(search.continuation_), followup(search.followup_) {}

    bool should_stop() {
        if (stopped) return true;
        if (limits.nodes && nodes >= limits.nodes) return stopped = true;
        if (limits.movetime.count() > 0 && (nodes & 63ULL) == 0 &&
            std::chrono::steady_clock::now() - start >= limits.movetime)
            return stopped = true;
        return false;
    }
    int sparse_score(const std::unordered_map<std::uint64_t, int>& table, std::uint64_t key) const {
        auto it = table.find(key);
        return it == table.end() ? 0 : it->second;
    }
    void bounded_sparse_add(std::unordered_map<std::uint64_t, int>& table, std::uint64_t key, int delta) {
        int& value = table[key];
    ...
    int capture_history_score(const Move& move) const { return capture_history[move_slot(move)] / CaptureHistoryScale; }
    int continuation_score(const Move* prev, const Move* prev2, const Move& move) const {
        int score = 0;
        if (prev && prev->path_length)
            score += sparse_score(continuation, continuation_key(*prev, move)) / ContinuationScale;
        if (prev2 && prev2->path_length)
            score += sparse_score(followup, continuation_key(*prev2, move)) / FollowupScale;
        return score;
    }
    bool is_countermove(const Move* prev, const Move& move) const {
        if (!prev || !prev->path_length) return false;
        auto it = countermoves.find(move_slot(*prev));
        return it != countermoves.end() && it->second == move;
    }
    void record_quiet_cutoff(const Move& move, Depth depth, int ply, const Move* prev,
                             const Move* prev2, const std::vector<Move>& quiets) {
        int bonus = std::max(1, depth * depth);
        bounded_add(history[move_slot(move)], bonus);
        if (prev && prev->path_length) {
            bounded_sparse_add(continuation, continuation_key(*prev, move), 2 * bonus);
            countermoves[move_slot(*prev)] = move;
            if (countermoves.size() > 16384) countermoves.clear();
        }
        if (prev2 && prev2->path_length)
            bounded_sparse_add(followup, continuation_key(*prev2, move), bonus);
        int penalty = std::max(1, bonus / 2);
        for (const auto& q : quiets) {
            if (q == move) continue;
            bounded_add(history[move_slot(q)], -penalty);
            if (prev && prev->path_length)
                bounded_sparse_add(continuation, continuation_key(*prev, q), -penalty);
            if (prev2 && prev2->path_length)
                bounded_sparse_add(followup, continuation_key(*prev2, q), -std::max(1, penalty / 2));
        }
        if (ply >= 0 && ply < MaxPly && killers[ply][0] != move) {
            killers[ply][1] = killers[ply][0];
            killers[ply][0] = move;
        }
    }
    void record_tactical_cutoff(const Move& move, Depth depth) {
        bounded_add(capture_history[move_slot(move)], std::max(1, depth * depth * 2));
    }
    int capture_pressure(Position& p) {
        Key key = p.search_key();
        auto it = pressure_cache.find(key);
        if (it != pressure_cache.end()) return it->second;
        int pressure = 0;
        for (const auto& move : generate_tactical_moves_info(p))
    ...
    int capture_pressure_for(const Position& p, Color side) {
        Position copy = p;
...
Search::Search(TranspositionTable& tt)
    : tt_(tt), history_(HistoryMoveSlots, 0), capture_history_(HistoryMoveSlots, 0) {}

void Search::clear_memory() {
    std::fill(history_.begin(), history_.end(), 0);
    std::fill(capture_history_.begin(), capture_history_.end(), 0);
    countermoves_.clear(); continuation_.clear(); followup_.clear();
    root_cache_.clear(); root_depth_cache_.clear();
}
...
Value Search::quiescence(Position& p, Value alpha, Value beta, int ply, Context& c) {
    ...
    auto info = generate_tactical_moves_info(p);
    ...
    for (const auto& cur : ordered) {
        ...
        if (score >= beta) {
            c.record_tactical_cutoff(cur.move, 1);
            return beta;
        }
...
Value Search::negamax(Position& p, Depth depth, Value alpha, Value beta, int ply, Context& c,
                      bool pv_node, const Move* prev, const Move* prev2, int extensions_used) {
    if (p.remaining_board_plies() == 0 || ply >= MaxPly) return evaluate(p);
    if (depth <= 0) return quiescence(p, alpha, beta, ply, c);
    ++c.nodes;
    c.seldepth = std::max(c.seldepth, ply);
    if (c.should_stop()) return evaluate(p);

    const Value original_alpha = alpha, original_beta = beta, static_eval = evaluate(p);
    const Key key = p.search_key();
    const TTEntry* entry = tt_.probe(key);
    Move tt_move{};
    const bool has_tt = entry && entry->has_move;
    if (has_tt) tt_move = entry->best_move;
    ...
    if (has_tt && p.is_legal_path(tt_move)) {
        int own = p.captures(mover), opp = p.captures(opposite(mover));
        UndoState undo;
        p.do_move(tt_move, undo);
        int swing = (p.captures(mover) - own) - (p.captures(opposite(mover)) - opp);
        bool extend = swing != 0 && extensions_used < MaxSelectiveExtensions;
        Value score = -negamax(p, depth - 1 + (extend ? 1 : 0), -beta, -alpha, ply + 1, c,
                               pv_node, &tt_move, prev, extensions_used + (extend ? 1 : 0));
        p.undo_move(undo);
        searched_tt = true;
        move_index = 1;
        if (c.stopped) return alpha;
        if (swing == 0) quiets.push_back(tt_move);
        if (score > alpha) { alpha = score; best = tt_move; has_best = true; }
        if (alpha >= beta) {
            if (swing == 0) c.record_quiet_cutoff(tt_move, depth, ply, prev, prev2, quiets);
            else c.record_tactical_cutoff(tt_move, depth);
            tt_.store(key, depth, alpha, Bound::Lower, &tt_move);
            return alpha;
        }
    }

    const auto info = generate_search_moves_info(p);
    if (info.empty()) {
        if (has_best) { tt_.store(key, depth, alpha, Bound::Exact, &best); return alpha; }
        return evaluate(p);
    }

    int own_pressure = 0;
    for (const auto& e : info) if (e.capture_swing > 0 && ++own_pressure >= 4) break;
    const bool inspect = depth >= 4;
    const int opponent_pressure = inspect ? c.capture_pressure_for(p, opposite(mover)) : 0;

    std::vector<OrderedMove> ordered;
    ordered.reserve(info.size());
    for (const auto& e : info) {
        const Move& m = e.move;
        if (searched_tt && m == tt_move) continue;
        int score = c.history_score(m) + c.continuation_score(prev, prev2, m);
        if (c.is_countermove(prev, m)) score += CountermoveBonus;
        if (e.capture_swing > 0) score += 500000 + e.capture_swing * 10000 + c.capture_history_score(m);
    ...
    for (const auto& cur : ordered) {
        ...
        Value score;
        if (move_index == 0) {
            score = -negamax(p, full, -beta, -alpha, ply + 1, c, pv_node, &cur.move, prev, next_ext);
        } else {
            int hist = c.history_score(cur.move) + c.continuation_score(prev, prev2, cur.move);
            bool known = c.is_countermove(prev, cur.move) ||
                         (ply < MaxPly && (is_killer(cur.move, c.killers[ply], 0) || is_killer(cur.move, c.killers[ply], 1))) ||
                         hist > 4 * depth * depth;
            bool reducible = !pv_node && cur.capture_swing == 0 && depth >= 4 && move_index >= 4 && !known;
            int reduction = 1;
            if (reducible) {
                int opp_after = c.capture_pressure(p), own_after = c.capture_pressure_for(p, mover);
                bool defensive = opponent_pressure > 0 && opp_after < opponent_pressure;
                bool threat = own_after > own_pressure;
                if (defensive || threat) reducible = false;
                if (reducible) {
                    Value child_static = -evaluate(p);
                    bool improving = child_static >= static_eval + 8;
                    if (!improving && cur.move.item == Item::None && depth >= 7 && move_index >= 10 && hist <= 0)
                        reduction = 2;
                }
            }
            if (reducible) {
                Depth rd = std::max<Depth>(0, full - reduction);
                score = -negamax(p, rd, -alpha - 1, -alpha, ply + 1, c, false, &cur.move, prev, next_ext);
                if (score > alpha)
                    score = -negamax(p, full, -alpha - 1, -alpha, ply + 1, c, false, &cur.move, prev, next_ext);
            } else {
                score = -negamax(p, full, -alpha - 1, -alpha, ply + 1, c, false, &cur.move, prev, next_ext);
            }
        ...
        ++move_index;
        if (c.stopped) return alpha;
        if (cur.capture_swing == 0) quiets.push_back(cur.move);
        if (score > alpha) { alpha = score; best = cur.move; has_best = true; }
        if (alpha >= beta) {
            if (cur.capture_swing == 0) c.record_quiet_cutoff(cur.move, depth, ply, prev, prev2, quiets);
            else c.record_tactical_cutoff(cur.move, depth);
            break;
        }
    }
...
SearchResult Search::run(Position root, const SearchLimits& limits) {
    ...
    Context c(*this);
    c.limits = limits;
    c.limits.multipv = std::clamp(c.limits.multipv, 1, 8);
    ...
    if (auto it = root_cache_.find(root_key); it != root_cache_.end()) prior = it->second;
    Depth cached_depth = 0;
    if (auto it = root_depth_cache_.find(root_key); it != root_depth_cache_.end()) cached_depth = it->second;
    std::unordered_map<Move, Value, MoveHash> prior_value;
    auto rebuild = [&] {
        prior_value.clear(); prior_value.reserve(prior.size() * 2 + 1);
        for (const auto& line : prior) prior_value[line.move] = line.value;
    };
    rebuild();
    ...
    if (!prior.empty()) {
        auto legal = std::find_if(prior.begin(), prior.end(), [&](const RootLine& line) {
    ...
    auto build_pv = [&](const Move& first, Depth depth) {
        ...
        Position p = root;
        Move m = first;
        int max = std::min(MaxPly, depth + MaxSelectiveExtensions + 2);
        for (int ply = 0; ply < max && m.path_length; ++ply) {
            if (!p.is_legal_path(m)) break;
            pv.push_back(m);
            UndoState undo;
            p.do_move(m, undo);
            const TTEntry* entry = tt_.probe(p.search_key());
            m = entry && entry->has_move ? entry->best_move : Move{};
        }
        return pv;
    };
    ...
    auto search_root = [&](Depth depth, Value alpha, Value beta, const std::vector<Move>& excluded, const Move& preferred) {
        ...
        for (const auto& m : root_moves) {
            if (is_excluded(m, excluded)) continue;
            int swing = root_swing[m];
            int score = c.history_score(m);
            if (m == preferred) score += 1200000;
        ...
        for (const auto& cur : ordered) {
            ...
            Value score;
            if (move_index == 0) {
                score = -negamax(root, child, -beta, -current_alpha, 1, c, true, &cur.move, nullptr, ext);
            } else {
                Value old = -Infinity;
                ...
                if (reduce) {
                    ...
                    if (score > current_alpha)
                        score = -negamax(root, child, -current_alpha - 1, -current_alpha, 1, c, false, &cur.move, nullptr, ext);
                } else {
                    score = -negamax(root, child, -current_alpha - 1, -current_alpha, 1, c, false, &cur.move, nullptr, ext);
                }
            ...
            root.undo_move(undo);
            ++move_index;
            if (c.stopped) return pass;
            pass.scores.push_back({cur.move, score, {}});
            if (score > pass.value) { pass.value = score; pass.best = cur.move; }
    ...
    for (Depth depth = start_depth; depth <= std::max(1, limits.depth); ++depth) {
        Value alpha = -Infinity, beta = Infinity, window = AspirationWindow;
        ...
        while (true) {
            ...
            if (pass.value <= alpha && alpha > -Infinity) {
                window *= 2; alpha = std::max(-Infinity, previous_value - window); beta = std::min(Infinity, previous_value + window); continue;
            }
    ...
    result.elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - c.start);
    return result;
}
}  // namespace rpsc
