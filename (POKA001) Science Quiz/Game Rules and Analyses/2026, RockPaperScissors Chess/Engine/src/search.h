...
struct SearchResult { Move best_move{}; bool has_move=false; Value value=0; Depth depth=0; Depth seldepth=0; std::uint64_t nodes=0; std::chrono::milliseconds elapsed{0}; std::vector<Move> pv; std::vector<RootLine> lines; };
class Search {
 public:
  explicit Search(TranspositionTable& tt); SearchResult run(Position position,const SearchLimits& limits); void clear_memory();
 private:
  TranspositionTable& tt_; std::vector<int> history_,capture_history_;
