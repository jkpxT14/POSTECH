...
int main() {
    ...
    for (Gesture gesture : {Gesture::Scissors, Gesture::Rock, Gesture::Paper}) {
        auto orientation = orientations.canonical(gesture, WristDirection::South);
        assert(orientations.top_gesture(orientation) == gesture);
        const auto north = orientations.roll(orientation, Direction::North);
        assert(orientations.roll(north, Direction::South) == orientation);
        for (int i = 0; i < 4; ++i) orientation = orientations.rotate_right(orientation);
        assert(orientations.top_gesture(orientation) == gesture);
    }
    ...
    bool saw_step_short = false;
    bool saw_step_long = false;
    ...
    for (const auto& line : item_result.lines) {
        assert(item_engine.position().is_legal_path(line.move));
        assert(!line.pv.empty());
    }
