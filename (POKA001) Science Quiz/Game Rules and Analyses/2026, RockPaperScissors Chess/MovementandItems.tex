...
\begin{center}
    ...
    \begin{tabular}{@{}L{0.17\textwidth}L{0.71\textwidth}@{}}
        ...
        아이템 & 실전 계산 \\
        \midrule
        Push & Gesture State는 바뀌지 않는다. 한 칸 민 뒤 $\ell(s)$번 Roll하며, 첫 Roll에서 Push 이전 칸으로 되돌아갈 수 있다. \\
...
Push 후 첫 Roll에서 시작칸으로 돌아오더라도 전체 이동이 끝난 것은 아니다. 예를 들어 East로 민 뒤 West로 한 번 Roll하면 위치는 돌아오지만 Gesture State는 $\tau_{\mathrm{NS}}(s)$가 된다. 이때 이미 한 번 Roll했으므로 남은 Roll 수는 $\ell(s)-1$이며, 다음 Roll에서 즉시 East로 역행할 수 없다.


Push 도해는 Push 후 첫 Roll의 복귀를 허용하고, 연속된 Roll 사이에만 즉시 역행 금지를 적용하여 계산한다. 도해의 격자는 시작칸 기준의 상대 도달 범위이며 실제 보드의 크기가 아니다. 실전에서는 보드 경계, 다른 말과 최종 대결 조건을 추가로 확인한다. Rotation 도해는 여섯 Rotation을 각각 먼저 적용하고, Rotation 후 Top Gesture에 따른 3/4/5 Roll의 모든 합법 경로를 합쳐 표시한다. Step 도해는 Step Short와 Step Long의 합법 경로를 함께 표시한다.
