...
RPSC 기보법은 체스의 algebraic notation과 PGN을 바탕으로 퀴즈 결과, 주사위 이동 경로, 아이템을 기록한다. 실제 경기 기록과 분석 표시는 구분하되 같은 Move Notation을 사용한다.
...
최초 선공 보드 역할을 \textbf{White}, 최초 후공 보드 역할을 \textbf{Black}이라 한다. White/Black은 학교와 독립된 역할이다. \texttt{.rpsc} header에서 \texttt{White}/\texttt{Black}은 controller, \texttt{WhiteTeam}/\texttt{BlackTeam}은 학교 배정을 기록한다. 각 보드 역할의 네 말에는 경기 내내 변하지 않는 고유 번호를 붙인다.
...
\[
\boxed{\text{\texttt{Q[}\POSTECH\texttt{, }\KAIST\texttt{]}}}
\]
...
양교의 정오 여부가 같으면 White의 이동을 먼저, Black의 이동을 다음에 적는다. 다음 예시는 White=\POSTECH{}, Black=\KAIST{}인 경우이다.
\begin{quote}\ttfamily
...
\medskip\noindent\textbf{Resignation.}
기권은 2026 Science War 공식 경기 규칙이 아니라, 선수단의 연습 및 분석 경기에서 한쪽이 더 이상 경기를 계속할 필요가 없다고 판단할 때 사용할 수 있는 종료 방식이다. 기권 자체는 Move Notation으로 기록하지 않는다. 승자를 기준으로 \texttt{Result}를 \texttt{1-0} 또는 \texttt{0-1}로 기록하고, 종료 사유를 남길 때에는 \texttt{Termination} header를 사용한다.
\begin{Verbatim}[fontsize=\small]
[Result "1-0"]
[Termination "Resignation"]
\end{Verbatim}
\texttt{Score}, \texttt{Quiz}, \texttt{Captures}는 기권이 이루어진 시점의 실제 값을 기록한다. 기권은 플레이어가 직접 선택하는 종료 방식이며 엔진 Evaluation에 따라 자동으로 적용하지 않는다.
...
\medskip\noindent\textbf{Incomplete Records.}
Analysis Board의 \texttt{.rpsc} 파일은 진행 중인 확정 상태에서도 저장할 수 있다. \texttt{Q[1,1]} 또는 \texttt{Q[0,0]} 뒤에 이동이 없으면 White 이동 전, White의 Move Notation만 있으면 Black 이동 전 상태를 뜻한다. 한 학교만 정답인 경우에는 정답 학교의 Item Choice가 남아 있을 수 있다. Analysis Board가 저장하는 Session metadata는 main line, sideline, 현재 분석 위치와 재사용 가능한 엔진 분석 결과를 보존하기 위한 보조 정보이며, 위의 canonical Game Record 표기 자체를 바꾸지 않는다.

현재 저장 형식인 \textbf{Format 3}에서 \texttt{Q}는 항상 \POSTECH--\KAIST{} 순서이고, \texttt{White}/\texttt{Black}은 controller, \texttt{WhiteTeam}/\texttt{BlackTeam}은 학교 배정이다. \texttt{QOrder}는 \texttt{POSTECH, KAIST}로 기록한다.
