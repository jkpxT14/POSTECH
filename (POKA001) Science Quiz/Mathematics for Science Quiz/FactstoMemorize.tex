    ...
    \begin{fact}[ICM and Fields Medal landmarks]
        ...
        \begin{itemize}
            ...
            \item Hong Wang became the third woman Fields Medalist in 2026.
            \item ICM 2014 was held in Seoul, Republic of Korea.
            ...
            \item ICM 2026: Philadelphia, United States, July 23--30, 2026.
            \item There is no ICM scheduled for 2027: the regular cycle proceeds
                  from 2026 to 2030.
            \item ICM 2030 is scheduled for Glasgow, United Kingdom; the associated IMU
    ...
    \begin{itemize}
        ...
        \item \textbf{2014.} ICM Seoul was held in the Republic of Korea, and Maryam
    ...
    \begin{theorem}[Bourgain's Slicing Problem]
        There exists a universal constant $c>0$ s.t. for every dimension $n$
        and every convex body $K\subseteq\R^n$ of volume $1$, there is an
        affine hyperplane $H$ s.t.
        \[
    ...
    \begin{conjecture}[$abc$ Conjecture]
        For every $\varepsilon>0$, there exists a constant $K_{\varepsilon}$ s.t., whenever $a,b,c$ are pairwise coprime positive integers satisfying
        $a+b=c$, we have
        \[
    ...
    \begin{conjecture}[Hadamard Conjecture]
        ...
        $\mathbf{H}$ with entries in $\{-1,1\}$ s.t.
        \[
    ...
    \begin{conjecture}[Sunflower Conjecture]
        ...
        s.t. every sufficiently large family of $w$-element sets with more than
        \[
            C_r^w
        \]
    ...
    \begin{conjecture}[Erd\H{o}s--Hajnal Conjecture]
        For every fixed graph $H$, there exists a constant $c(H)>0$ s.t. every
    ...
    \begin{remark}
        ...
        proof that Gerver's construction is optimal. At the September 1, 2026
        preparation cutoff, the result remained a preprint, so its publication
        status should
    ...
    \begin{fact}[Jacobian Conjecture: 2026 Status]
        Keller's 1939 conjecture asserted that a polynomial map
        \[
        ...
        \[
            ...
            \right)
        \]
        must have a polynomial inverse.

        In July 2026, Levent Alpöge announced an explicit counterexample in
        dimension $3$: the Jacobian determinant is a nonzero constant, yet the
        polynomial map is not injective. An independent Isabelle/HOL
        formalization verified the explicit map and its noninjectivity. Further
        constructions give counterexamples in every dimension $n>2$.

        Therefore the classical Jacobian conjecture is false for
        \[
            n\geq 3,
        \]
        while the planar case $n=2$ remains open. The announcement credits
        Akhil Mathew with prompting the question and the AI system Claude Fable
        with work leading to the counterexample; it should not be summarized as
        ``AI alone solved the Jacobian conjecture.''
    \end{fact}


    \begin{fact}[Erd\H{o}s--Szemer\'edi Sum--Product Conjecture: 2026 Status]
        A standard real-variable form predicted that every sufficiently large
        finite set $A\subset\R$ must have either an almost-quadratically large
        sumset $A+A$ or an almost-quadratically large product set $AA$. In May
        2026, Thomas F. Bloom, Will Sawin, Carl Schildkraut, and Dmitrii Zhelezov
        disproved this statement over $\R$: for some absolute constant $c>0$
        there are arbitrarily large finite sets $A\subset\R$ with
        \[
            \max\{|A+A|,|AA|\}\leq |A|^{2-c}.
        \]
        Their construction uses algebraic integers in number fields whose degree
        grows with $|A|$. It therefore does not disprove the original integer
        version, which may still hold for $A\subset\Z$.
    \end{fact}

    \begin{fact}[Cycle Double Cover Conjecture: 2026 Status]
        The Cycle Double Cover Conjecture states that every finite bridgeless
        loopless multigraph has a multiset of cycles in which every edge occurs
        exactly twice. In July 2026, OpenAI announced an affirmative proof
        generated with GPT-5.6 Sol Ultra, and expository preprints by Sang-il
        Oum and Jim Geelen presented the argument in conventional mathematical
        form. At the September 1, 2026 preparation cutoff, the result should be
        remembered as an announced and publicly exposited proof rather than
        silently conflated
        with ordinary journal publication.
    \end{fact}

    \begin{fact}[Petersen Coloring Conjecture: 2026 Status]
        The Petersen Coloring Conjecture asserted that every bridgeless cubic
        graph admits a Petersen coloring. In August 2026, Bryce Putman produced
        an explicit simple bridgeless cubic counterexample on $112$ vertices,
        supported by SAT formulations and independently checkable DRAT
        certificates. Jorik Jooken then gave a human-checkable proof for the
        same counterexample. Thus the conjecture is false.
    \end{fact}

    \begin{fact}[Heil--Ramanathan--Topiwala Conjecture: 2026 Status]
        The Heil--Ramanathan--Topiwala, or HRT, Conjecture asserted that for
        every nonzero $f\in L^2(\R)$ and every finite set of distinct
        time--frequency shifts, the resulting functions are linearly
        independent. In August 2026, Markus Faulhuber, Philipp Petersen, Jordy
        Timo van Velthoven, and Felix Voigtlaender disproved the conjecture with
        $12$ shifts of a Schwartz function. Vignon Oussa then gave a
        computer-assisted $4$-shift counterexample. Thus the HRT conjecture is
        false.
    \end{fact}

    \begin{fact}[Sendov's Conjecture: 2026 Status]
        Sendov's conjecture states that if a polynomial $p$ of degree $n\geq2$
        has all of its zeros in the closed unit disk, then for every zero $a$ of
        $p$ there exists a critical point $\zeta$ of $p$ s.t.
        \[
            p'(\zeta)=0,
            \qquad
            |\zeta-a|\leq1.
        \]
        In August 2026, Lech Mazur obtained an AI-assisted proof covering all
        degrees. Terence Tao subsequently produced a human-digested exposition
        and a streamlined Lean formalization, and explained that the same ideas
        establish the stronger Phelps--Rodriguez form. At the September 1, 2026
        preparation cutoff, the mathematical argument had therefore received
        both human exposition and formal verification; this status should still
        be distinguished from ordinary journal publication.
    \end{fact}
