    ...
    otherwise stated, vectors are column vectors. Number systems are written in
    blackboard bold, sets ordinarily use uppercase italic letters, and families
    and collections use calligraphic letters when useful. Named operators are
    set upright.

    Round parentheses are preferred for ordered tuples, sequences, and matrix
    delimiters; square brackets are an accepted alternative for matrices. Square
    brackets are also used for closed intervals and for area notation such as
    ...
    used in ordinary prose. The symbol $\Delta$ follows standard mathematical
    and physical usage: $\Delta q$ may denote a finite change in a quantity,
    $\Delta a_n=a_{n+1}-a_n$ denotes the forward difference of a sequence, and
    $\Delta f$ denotes the Laplacian when $\Delta$ acts as a differential
    operator on a function. These meanings are distinguished by context and, when
    more than one occurs nearby, the intended meaning is stated explicitly.
    When several
    equivalent notations are listed in the same table entry, the first notation is
    the convention used in this book; the later forms are included only to identify
    common alternatives that a reader may encounter elsewhere.
    ...
    \begin{longtable}{@{}L{0.24\textwidth}L{0.68\textwidth}@{}}
        ...
        $\Rbar=\R\cup\{-\infty,+\infty\}$ & the set of extended real numbers \\
        $\C$ & the set of complex numbers \\
        $\Hquat$ & the quaternion division ring \\
        $a\coloneqq b$ & $a$ is defined to be $b$ \\
        \textnormal{s.t.} & abbreviation used to introduce a condition or constraint \\
        ...
        $\sgn(x)$ & the sign of a real number or a permutation, according to context \\
    ...
    \begin{longtable}{@{}L{0.24\textwidth}L{0.68\textwidth}@{}}
        ...
        Definition & the precise meaning of a term, object, property, or construction \\
        Theorem & a principal mathematical result \\
        Proposition & a useful result of more limited scope \\
        Lemma & a supporting result, usually for a later proof \\
        Corollary & a result following readily from a preceding result \\
        Conjecture & a statement proposed to be true but not proved in full generality \\
        Hypothesis & a named assumption or proposed statement \\
        Formula & a distinguished computational identity or closed form \\
        Algorithm & an explicit step-by-step procedure \\
        Fact & a verified statement not requiring theorem status \\
        Example & an illustration of a definition, construction, phenomenon, or result \\
        Counterexample & an example disproving a general assertion or showing that a hypothesis is necessary \\
        Remark & supplementary context, a warning, or a qualification \\
        Problem & a mathematical task intended to be solved \\
        Open Problem & a problem not known to be solved in the stated form \\
        Idea & a decisive conceptual insight \\
        Strategy & a broad problem-solving plan \\
        Tactic & a local step within a larger strategy \\
    \end{longtable}
    }

    Proof labels are written as \textit{Proof} and solution labels as
    \textit{Solution}, each followed by a period. The ordinary end mark is
    $\square$; $\blacksquare$ is reserved for substantial or especially
    important proofs, and \textsc{Q.E.D.} for exceptional paper-level proofs.

    Enumeration labels are set upright. Decimal labels such as $1.1,1.2,\dots$ identify
    independent units under a common parent; $(1),(2),\dots$ are used for
    ordinary enumerations; $(a),(b),\dots$ are used for connected subparts of
    one problem; and $(i),(ii),\dots$ are reserved for formal mathematical
    clauses or hypotheses. Displayed equations are punctuated as parts of their
    surrounding sentences and are numbered only when they are referenced or
    need a stable label.
    ...
    indicates another meaning. The arrow $f\colon X\to Y$ specifies the domain and
    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $\displaystyle\bigcup_{i\in I}A_i$ & the union of an indexed family of sets \\
        $\displaystyle\bigcap_{i\in I}A_i$ & the intersection of an indexed family of sets \\
        ...
        $f\colon X\hookrightarrow Y$ & an injective map \\
        $f\colon X\twoheadrightarrow Y$ & a surjective map \\
        $f\colon X\bijarrow Y$ & a bijective map \\
        $f\colon X\xrightarrow{\sim}Y$ & an isomorphism \\
        ...
        $\indicator{A}(x)$ & the indicator function of $A$ \\
        $\aleph_0$ & the cardinality of a countably infinite set \\
        $\mathfrak{c}$ & the cardinality of the continuum \\
    \end{longtable}
    }

    The symbol $\indicator{A}$ denotes an indicator function, not a vector.
    The all-ones vector is $\ones$. Boldface is otherwise reserved for vectors
    and matrices.
    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $\displaystyle\lim_{x\to a^-}f(x)$ & the left-hand limit of $f(x)$ as $x$ approaches $a$ \\
        $\displaystyle\lim_{x\to a^+}f(x)$ & the right-hand limit of $f(x)$ as $x$ approaches $a$ \\
        $\min A$, $\max A$ & the minimum and maximum of $A$ \\
        $\inf A$, $\sup A$ & the infimum and supremum of $A$ \\
        ...
        $\displaystyle\iint_D f\dd A$ & a double integral over $D$ \\
        $\displaystyle\iiint_\Omega f\dd V$ & a triple integral over $\Omega$ \\
        $\displaystyle\sum_{k=1}^{n}a_k$ & a finite sum \\
        $\displaystyle\sum_{k=1}^{\infty}a_k$ & an infinite series \\
        $\displaystyle\prod_{k=1}^{n}a_k$ & a finite product \\
        ...
        $\arcsec x$ & the principal inverse secant, with values in $[0,\pi]\setminus\{\pi/2\}$ \\
        $\arccsc x$ & the principal inverse cosecant, with values in $[-\pi/2,\pi/2]\setminus\{0\}$ \\
        $\arccot x$ & the principal inverse cotangent, with values in $(0,\pi)$ \\
        $\sech x$ & the hyperbolic secant $1/\cosh x$ \\
        $\csch x$ & the hyperbolic cosecant $1/\sinh x$ \\
        $\arcsinh x$ & the inverse hyperbolic sine \\
        $\arccosh x$ & the principal inverse hyperbolic cosine, with values in $[0,\infty)$ \\
        $\arctanh x$ & the inverse hyperbolic tangent on $(-1,1)$ \\
        $\arcsech x$ & the principal inverse hyperbolic secant on $(0,1]$ \\
        $\arccsch x$ & the principal inverse hyperbolic cosecant on $\R\setminus\{0\}$ \\
        $\arccoth x$ & the principal inverse hyperbolic cotangent on $(-\infty,-1)\cup(1,\infty)$ \\
        ...
        $\Delta q=q_{\mathrm{final}}-q_{\mathrm{initial}}$ & the finite change of a quantity $q$, when the context is discrete or physical \\
        $\Delta f$ & the Laplacian of $f$, when $\Delta$ acts as a differential operator \\
        ...
        $\RePart z$, $\Re z$ & the real part of $z$ \\
        $\ImPart z$, $\Im z$ & the imaginary part of $z$ \\
        $\overline{z}$ & the complex conjugate of $z$ \\
        $|z|$ & the modulus of $z$ \\
        $\arg z$ & the multivalued argument of $z\neq0$ \\
        $\Arg z$ & the principal argument, with $-\pi<\Arg z\leq\pi$ \\
        $\log z$ & the multivalued complex logarithm \\
        $\Log z=\ln|z|+i\Arg z$ & the principal complex logarithm \\
    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $\diag(d_1,\dots,d_n)$ & the diagonal matrix with diagonal entries $d_1,\dots,d_n$ \\
        ...
        $\im T$ & the image of a linear map $T$ \\
        ...
        $\mathbf{u}\cdot\mathbf{v}$ & the standard Euclidean inner product on $\R^n$ \\
        $\mathbf{u}^{\mathsf H}\mathbf{v}$ & the standard Hermitian inner product on $\C^n$ \\
        $\|\mathbf{v}\|$ & the norm of $\mathbf{v}$ \\
        $U^\perp$ & the orthogonal complement of the subspace $U$ \\
        $\proj_U\mathbf{x}$ & the orthogonal projection of $\mathbf{x}$ onto $U$ \\
        ...
        $E_\lambda(\mathbf{A})$ & the eigenspace of $\mathbf{A}$ associated with $\lambda$ \\
        $\sigma(\mathbf{A})$ & the spectrum of $\mathbf{A}$ \\
    \end{longtable}
    }

    On a complex inner-product space, the inner product is conjugate-linear in
    its first argument and linear in its second argument.
    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $W(y_1,\dots,y_n)(t)$ & the Wronskian of $y_1,\dots,y_n$ \\
        $J_\nu(x)$, $Y_\nu(x)$ & Bessel functions of the first and second kinds of order $\nu$ \\
        $\mathcal{L}\{f(t)\}(s)$ & the Laplace transform of $f(t)$ \\
        $H(t-a)$, $u(t-a)$ & the Heaviside unit-step function shifted by $a$ \\
        $\widehat f(\boldsymbol{\xi})$ & the Fourier transform under the normalization above \\
    \end{longtable}
    }
    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $\multichoose{n}{k}$, ${}_nH_k$ & the number of size-$k$ multisets chosen from an $n$-element set \\
    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $X\sim\Bern(p)$ & a Bernoulli distribution \\
        $X\sim\Bin(n,p)$ & a binomial distribution \\
        $X\sim\Geom(p)$ & a geometric distribution on $\N$, counting the successful trial \\
        $X\sim\Poi(\lambda)$ & a Poisson distribution \\
        $X\sim\Unif(a,b)$ & a continuous uniform distribution on $[a,b]$ \\
        $X\sim\ExpDist(\lambda)$ & an exponential distribution with rate $\lambda$ \\
    ...
    side lengths opposite $A,B,C$, respectively.
    ...
    \begin{longtable}{@{}L{0.24\textwidth}L{0.68\textwidth}@{}}
        ...
        $\lengthop(S)$ & the length of a curve or one-dimensional geometric object $S$ \\
        $\area(S)$ & the area of a planar region or surface $S$ \\
        $\vol(S)$ & the volume of a three-dimensional region or solid $S$ \\
        $\dist(P,S)$ & the distance from the point $P$ to the set or geometric object $S$ \\
        $\conv(S)$ & the convex hull of $S$ \\
        $\R^n$ & $n$-dimensional Euclidean space, in geometric contexts \\
        $\mathbb{H}^n$ & $n$-dimensional hyperbolic space, in geometric contexts;\newline $\mathbb{H}$ without an exponent denotes the quaternions in algebraic contexts \\
    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $\lcmop(a,b)$ & the least common multiple of $a$ and $b$ \\
        ...
        $\rad(n)$ & the product of the distinct prime divisors of $n$ \\
    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $N\rtimes_\varphi H$ & the semidirect product determined by $\varphi\colon H\to\Aut(N)$ \\
        $\ker\varphi$ & the kernel of a homomorphism $\varphi$ \\
        $\im\varphi$ & the image of a homomorphism $\varphi$ \\
        ...
        $\trdeg_K L$ & the transcendence degree of $L$ over $K$ \\
        $\charac F$ & the characteristic of $F$ \\
    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $X\vee Y$ & the wedge sum of pointed spaces $X$ and $Y$ \\
        $\pi_1(X,x_0)$ & the fundamental group of $X$ based at $x_0$ \\
        $\chi(X)$ & the Euler characteristic of $X$ \\
        $X\cong Y$ & $X$ and $Y$ are homeomorphic \\
        $X\simeq Y$ & $X$ and $Y$ are homotopy equivalent \\
    \end{longtable}
    }
    ...
    \begin{longtable}{@{}L{0.12\textwidth}L{0.22\textwidth}L{0.38\textwidth}L{0.20\textwidth}@{}}
        ...
        $\sqrt{3}$
        & Square root of $3$
        & The positive square root of $3$.
        & $1.73205080757\ldots$ \\

        $\sqrt{5}$
        & Square root of $5$
        & The positive square root of $5$.
        & $2.23606797750\ldots$ \\

        $\sqrt{7}$
        & Square root of $7$
        & The positive square root of $7$.
        & $2.64575131106\ldots$ \\
