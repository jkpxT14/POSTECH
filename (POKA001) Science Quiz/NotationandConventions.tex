    ...
    clearly indicates another meaning. The arrow $f:X\to Y$ specifies the
    domain and codomain of a function, while $x\mapsto f(x)$ specifies its
    ...
    and $\Delta$ denote the circumradius, inradius, semiperimeter, and area.
    ...
    The symbols below are the standard ones most often used in mathematics.
    When a named constant has no completely universal single-letter symbol, its
    defining expression is preferred.

    \vspace{0.5em}

    {\small
    \renewcommand{\arraystretch}{1.18}
    \begin{longtable}{@{}L{0.12\textwidth}L{0.21\textwidth}L{0.37\textwidth}L{0.22\textwidth}@{}}
        \toprule
        Symbol & Name & Meaning or definition & Exact value or decimal approximation \\
        \midrule
        \endfirsthead
        ...
        $i$
        & Imaginary unit
        & The distinguished square root of $-1$ in $\C$.
        & $i^2=-1$ \\

        $\sqrt{2}$
        & Pythagoras constant
        & The positive length of the diagonal of a unit square.
        & $1.414213562373095\ldots$ \\
        ...
        $\pi+e$
        & Sum of pi and Euler's number
        & A useful quiz combination of the two most familiar transcendental
          constants.
        & $5.859874482048838\ldots$ \\

        $\varphi$
        & Golden ratio
        & The positive solution of $x^2=x+1$.
        & $\dfrac{1+\sqrt{5}}{2}=1.618033988749895\ldots$ \\
        ...
        $K$
        & Khinchin's constant
        & The almost-sure limiting geometric mean of the partial quotients in
          the continued fraction of a real number.
        & $2.685452001065306\ldots$ \\

        $A$
        & Glaisher--Kinkelin constant
        & A constant arising in products of powers and in values of the
          derivative of the Riemann zeta function.
        & $1.282427129100623\ldots$ \\

        $\Omega$
        & Omega constant
        & The real solution of $x e^x=1$, so
          $\Omega=W(1)$ for the Lambert $W$ function.
        & $0.567143290409784\ldots$ \\

        $\rho$
        & Plastic constant
        & The unique real solution of $x^3=x+1$.
        & $1.324717957244746\ldots$ \\

        $\delta$
        & Feigenbaum delta
        & The universal limiting ratio of parameter intervals in
          period-doubling bifurcations.
        & $4.669201609102990\ldots$ \\

        $|\alpha|$
        & Feigenbaum alpha magnitude
        & The universal spatial scaling magnitude in period-doubling
          bifurcations.
        & $2.502907875095893\ldots$ \\

        $B_1$
        & Meissel--Mertens constant
        & $\displaystyle
          B_1=\lim_{x\to\infty}
          \left(\sum_{p\leq x}\frac{1}{p}-\log\log x\right)$.
        & $0.261497212847643\ldots$ \\

        $C_2$
        & Twin-prime constant
        & $\displaystyle
          C_2=\prod_{p>2}\frac{p(p-2)}{(p-1)^2}$.
        & $0.660161815846870\ldots$ \\

        $B_2$
        & Brun's constant
        & The convergent sum of reciprocals over twin-prime pairs:
          $\displaystyle \sum_{p,\,p+2\ \mathrm{prime}}
          \left(1/p+1/(p+2)\right)$.
        & $1.902160583104\ldots$ \\

        $C_{10}$
        & Champernowne constant
        & The base-$10$ number formed by concatenating the positive integers.
