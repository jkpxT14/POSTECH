    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $f_n\ptwto f$ & pointwise convergence of functions \\
        $f_n\rightrightarrows f$ & uniform convergence of functions \\
        $f_n\unifto f$ & an explicitly labeled alternative for uniform convergence \\
        $f_n\aeto f$ & convergence almost everywhere \\
        $\|f\|_\infty$ & the supremum norm of $f$ \\
    \end{longtable}
    }

    The ordinary arrow $\to$ is used for sequence limits and for functions when
    no special property is being encoded. There is no single universal special
    arrow for pointwise convergence, so this book uses the explicit label
    $\ptwto$. The double arrow $\rightrightarrows$ is a common, though not
    universal, convention for uniform convergence; it is adopted consistently in
    this book. The labeled alternative $\unifto$ may be used when several modes
    of convergence occur in the same passage.
    ...
    \begin{longtable}{@{}L{0.27\textwidth}L{0.65\textwidth}@{}}
        ...
        $X_n\xrightarrow{\Prob}X$ & convergence in probability \\
