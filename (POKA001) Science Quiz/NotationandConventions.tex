    ...
    Within problems, independent choices, clues, assumptions, and ordered
    statements are labeled $(1),\ (2),\ (3),\ldots$. Connected subparts of a
    single problem are labeled $(a),\ (b),\ (c),\ldots$. Both kinds of lists are
    left aligned, use identical indentation and vertical spacing throughout Past
    Problems and Expected Problems, and align continuation lines with the text
    rather than with the label. The decimal labels $1.1,\ 1.2,\ 1.3,\ldots$ are
    reserved for independently movable calculation problems grouped under one
    Direct Calculation Problems category.

    A parenthetical attribution after a problem number is included only when it
    gives an independently verifiable official source or an established
    standard problem name. Internal preparation materials, category labels, and
    invented source descriptions are not cited in this position; when no
    qualifying source or standard name is known, the attribution is omitted.
    Direct Calculation Problems category titles are printed as structural
    headings rather than parenthetical attributions.
